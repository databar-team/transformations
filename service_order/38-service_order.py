 sql = """
              SELECT jso.service_order_id as SERVICE_ORDER_NUMBER,
...
date(date_format(jso.start_date_active, 'dd-MM-yyyy')) as start_date_active,
date(date_format(jso.end_date_active, 'dd-MM-yyyy')) as END_DATE_ACTIVE,
...
date(date_format(oa.start_date_active, 'dd-MM-yyyy')) as AGREEMENT_START_DATE_ACTIVE,
date(date_format(oa.end_date_active, 'dd-MM-yyyy')) as AGREEMENT_END_DATE_ACTIVE,
...
date(date_format(jso.creation_date, 'dd-MM-yyyy HH:mm:ss')) as CREATION_DATE,
so_creation.user_name as FIRST_ACTIVATED_BY,
date(so_creation.creation_date) as FIRST_ACTIVATION_DATE,
...
INNER JOIN rawdb.joe_service_orders jsojd
ON jso.service_order_id = jsojd.service_order_id
LEFT JOIN
(  SELECT service_order_id,
MAX(CASE site_use_code WHEN 'BILL_TO' THEN site_use_id ELSE NULL END)
bill_to_site_use_id,
MAX(CASE site_use_code WHEN 'SHIP_TO' THEN site_use_id ELSE NULL END)
ship_to_site_use_id,
MAX(CASE site_use_code WHEN 'SOLD_TO' THEN site_use_id ELSE NULL END)
sold_to_site_use_id,
MAX(CASE site_use_code WHEN 'BILL_TO' THEN contact_id ELSE NULL END)
bill_to_contact_id,
MAX(CASE site_use_code WHEN 'SHIP_TO' THEN contact_id ELSE NULL END)
ship_to_contact_id,
MAX(CASE site_use_code WHEN 'SOLD_TO' THEN contact_id ELSE NULL END)
sold_to_contact_id,
MAX (last_update_date) max_last_update_date
FROM rawdb.joe_so_addresses
...
LEFT JOIN rawdb.oe_lookups ol
ON     ol.lookup_code = oa.agreement_type_code
AND ol.lookup_type = 'AGREEMENT_TYPE'
LEFT JOIN rawdb.oe_price_lists_vl opl ON opl.price_list_id = oa.price_list_id
...
LEFT JOIN rawdb.jfn_common_lookups jcl_pack
...
FROM rawdb.RA_TERMS_TL T, rawdb.RA_TERMS_B B
WHERE B.TERM_ID = T.TERM_ID AND T.LANGUAGE = 'US') rt
ON rt.term_id = oa.term_id
LEFT JOIN rawdb.fnd_flex_values_vl s_program
ON     s_program.flex_value = jso.so_program
AND s_program.flex_value_set_id = 1002715     --flex_value_set_id
LEFT JOIN rawdb.fnd_user fndu ON fndu.user_id = jso.created_by
LEFT JOIN
(SELECT DISTINCT
jsoh.service_order_id, /*use distinct because service_order_id can have exact same create date in joe_so_status_history*/
fndu1.user_name,
date_format(jsoh.creation_date, 'dd-MM-yyyy HH:mm:ss')
creation_date
