...
cte_su 
AS (SELECT service_order_id, contact_id,
    ...
    WHERE 
        site_use_code IN ('BILL_TO', 'SOLD_TO', 'SHIP_TO')
    GROUP BY 
        service_order_id, contact_id
    )
SELECT
   ...
   (select last_name FROM cte_ra_contacts, rawdb.joe_so_addresses jsa RIGHT OUTER JOIN cte_su su on jsa.contact_id = su.ship_to_contact_id) as SHIP_TO_CONTACT,
...
LEFT JOIN cte_su
          on cte_su.contact_id = jsa.contact_id
LEFT JOIN cte_ra_contacts
          on cte_ra_contacts.CONTACT_ID = jsa.CONTACT_ID
