==== Upustvo za RA_CONTACTS

for transformation purposes, you can replace references to ra_contacts with sql below:
...
SELECT jso.service_order_id,
       ...
       (SELECT    last_name
               || DECODE (first_name, NULL, NULL, ', ' || first_name)
          FROM ra_contacts
         WHERE contact_id(+) = su.sold_to_contact_id)
          sold_to_contact,
       (SELECT    last_name
               || DECODE (first_name, NULL, NULL, ', ' || first_name)
          FROM ra_contacts
         WHERE contact_id(+) = su.bill_to_contact_id)
          bill_to_contact,
       ...
       fu.user_name,
       TO_CHAR (TRUNC (jso.start_date_active), 'DD-MON-YYYY')
          start_date_active,
       TO_CHAR (TRUNC (jso.end_date_active), 'DD-MON-YYYY')
          end_date_active,
       ...
       rt.name
          AGREEMENT_TERMS,
       TO_CHAR (TRUNC (oa.start_date_active), 'DD-MON-YYYY')
          AGREEMENT_START_DATE_ACTIVE,
       TO_CHAR (TRUNC (oa.end_date_active), 'DD-MON-YYYY')
       ...
       fndu.user_name,
       TO_CHAR (jso.creation_date, 'DD-MON-YYYY HH24:MI:SS')
          AS CREATION_DATE,
       ...
       --                     fndu1.user_name
       --                  || CHR (31)
       --                  || TO_CHAR (jsoh.creation_date, 'DD-MON-YYYY HH24:MI:SS'))
       --          FROM fnd_user fndu1, joe_so_status_history jsoh
       --         WHERE     jsoh.service_order_id = jso.service_order_id
       --               AND jsoh.status = 'ACTIVE'
