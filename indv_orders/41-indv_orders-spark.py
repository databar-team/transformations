sql = """WITH CTES AS (SELECT HCA.ACCOUNT_NUMBER, HCA.CUST_ACCOUNT_ID FROM rawdb.HZ_CUST_ACCOUNTS HCA)
...
joha.received_date as ORDER_ENTERED_DATE,
joha.requested_delivery_date as ORDER_REQUESTED_DELIVERY_DATE,
joha.requested_ship_date as ORDER_SCHEDULED_SHIP_DATE,
joha.creation_date as ORDER_CREATED_DATE,
cast(NULL as DOUBLE) as ORDER_IMPORTED_DATE,
cast(NULL  as DOUBLE) as ORDER_CYCLE_DURATION ,
...
CASE joha.context WHEN 'DIPL' THEN joha.attribute2 ELSE NULL END as DIPL_ORDER_NUMBER,
...
REPLACE (joha.comments, CHR (31), NULL) as COMMENTS,
...
joha.copied_from_order_header_id as COPIED_FROM_BRIDGE_ORDER,
CASE joha.context WHEN 'DIPL' THEN joha.attribute5 ELSE NULL END as BRIDGE_ORDER_GRAD_DATE,
...
CASE joha.context WHEN 'YBMS' THEN joha.attribute6 ELSE NULL END as YBMS_PAGES,
CASE joha.context WHEN 'YBMS' THEN joha.attribute7 ELSE NULL END as YBMS_COPIES,
CASE joha.context WHEN 'YBMS' THEN joha.attribute8 ELSE NULL END as YBMS_LARGE_LABEL_QTY,
CASE joha.context WHEN 'YBMS' THEN joha.attribute9 ELSE NULL END as YBMS_SMALL_LABEL_QTY,
CASE joha.context WHEN 'YBMS' THEN joha.attribute10 ELSE NULL END as YBMS_YEARTECH_ONLINE,
CASE joha.context WHEN 'YBMS' THEN joha.attribute11 ELSE NULL END as YBMS_SERVICE_ORDER_ID,
CASE joha.context WHEN 'YBMS' THEN joha.attribute12 ELSE NULL END as YBMS_MF_KIT_REQUES_DATE,
CASE joha.context WHEN 'YBMS' THEN joha.attribute13 ELSE NULL END as YBMS_COMMISSION_PCT,
CASE joha.context WHEN 'YBMS' THEN joha.attribute14 ELSE NULL END as YBMS_YTO_INDICATOR,
CASE joha.context WHEN 'DIPL' THEN joha.attribute6 ELSE NULL END as OP_RECEIVED_DATE,
...
SUBSTR (CASE joha.attribute17 WHEN 'Y' THEN CASE joha.context WHEN 'JLRY' THEN joha.attribute22 WHEN 'ANNC' THEN joha.attribute22 ELSE NULL END ELSE NULL END,1,49) 
 as ZDP_STATUS,
...
LEFT JOIN CTES c
          ON c.CUST_ACCOUNT_ID = cast(joha.ATTRIBUTE18 as bigint)"""
...
### Save DF to S3: 
df.write.mode("overwrite").parquet("s3://jostens-data-dev-test/indv_orders/") 
### Read From S3: 
df_check = spark.read.parquet("s3://jostens-data-dev-test/indv_orders/") 
df_check.count() 
df_check.show(10)  