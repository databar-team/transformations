...
date_format(joha.received_date, '%d-%m-%Y') as ORDER_ENTERED_DATE,
date_format(joha.requested_delivery_date, '%d-%m-%Y') as ORDER_REQUESTED_DELIVERY_DATE,
date_format(joha.requested_ship_date, '%d-%m-%Y') as ORDER_SCHEDULED_SHIP_DATE,
date_format(joha.creation_date, '%d-%m-%Y  %H:%i:%s') as ORDER_CREATED_DATE,
...
date_format(joha.LAST_UPDATE_DATE, '%d-%m-%Y') as LAST_UPDATE_DATE,
